...
async def test_setup_agent_bash_unavailable() -> None:
    ...
    with patch("supporter.tools.check_bash_availability", return_value=False):  # noqa: SIM117
        with patch("supporter.tools.notify_bash_unavailable") as mock_notify:
            with patch("supporter.get_provider"):  # noqa: SIM117
                with patch("supporter.agent.ChatAgent"):
                    await manager.setup_agent(use_crew=False, use_live=False)
...
@pytest.mark.asyncio
async def test_toggle_mode_crew() -> None:
    app = MagicMock()
    app.crew_mode = False
    app.live_mode = False
    ...
    manager = ModeManager(app)
    with patch.object(manager, "setup_agent", new_callable=AsyncMock):  # noqa: SIM117
        with patch.object(manager, "_update_ui_state"):
...
@pytest.mark.asyncio
async def test_toggle_mode_live() -> None:
    app = MagicMock()
    app.crew_mode = False
    app.live_mode = False
    app._start_thinking = MagicMock()
