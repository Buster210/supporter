...
def test_parse_break_range_defaults_and_bounds():
...
def test_human_break_draws_fresh_random_each_run():
    ...
    with (
        ...
        patch("supporter.recipes.time.sleep") as slept,
    ):
        got = [
            _execute_step(0, step, {"emitted": [], "name": "t"})["break_ms"]
            for _ in range(3)
        ]
...
def test_human_break_blank_uses_defaults():
    step = RecipeStep(kind="human_break", value="")
    with (
        patch("supporter.recipes.random.randint", return_value=900) as rnd,
        patch("supporter.recipes.time.sleep"),
    ):
        r = _execute_step(0, step, {"emitted": [], "name": "t"})
    assert r["ok"] is True
