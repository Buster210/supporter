    ...
    *,
    ...
    while round_no < rounds:
        ...
        for query in queries:
            try:
                results = await _search_results(query, per_query)
            except ToolError:
                raise
            ...
            for r in results[:per_query]:
                ...
                pages += 1
                try:
                    claims = await _extract_claims(provider, question, url, text)
                except Exception as exc:
                    logger.debug(f"deep_research: extract failed for {url}: {exc}")
                    continue
                ingested += ingest_claims(
                    qid, task_id=f"round-{round_no}", agent="deep_research",
                    claims=claims, round=round_no,
                )
        ...
        gaps = (
            assess["uncorroborated_statements"] + assess["conflicted_statements"]
        )
